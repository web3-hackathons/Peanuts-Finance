...
describe("Vaults", function () {
  ...
  beforeEach(async function () {
    ...
    //get artifacts
    Vault = await ethers.getContractFactory("PeanutsVault");
  ...
  describe("Vault Tests", function () {
    ...
    xit("should mint user their pool share", async function () {
      ...
      await want
        .connect(wantHolder)
        .transfer(owner.address, ownerDepositAmount);
      await want
        .connect(owner)
        .approve(vault.address, ethers.constants.MaxUint256);
      ...
      expect(ownerVaultBalance).to.be.closeTo(
        ownerDepositAmount,
        allowedImprecision
      );
    ...
    xit("should handle small deposit + withdraw", async function () {
      ...
      const expectedBalance = userBalance.sub(withdrawFee);
      const isSmallBalanceDifference =
        expectedBalance.sub(userBalanceAfterWithdraw) < 200;
      expect(isSmallBalanceDifference).to.equal(true);
    });
  ...
  describe("Strategy", function () {
    xit("should be able to pause and unpause", async function () {
      ...
      const depositAmount = toWantUnit("0.0000029");
      await expect(vault.connect(wantHolder).deposit(depositAmount)).to.be
        .reverted;

      await strategy.unpause();
      await expect(vault.connect(wantHolder).deposit(depositAmount)).to.not.be
        .reverted;
    });
    ...
    xit("should be able to panic", async function () {
      ...
      const allowedImprecision = toWantUnit("0.00000000001");
      expect(strategyBalance).to.be.closeTo(
        wantStratBalance,
        allowedImprecision
      );
    });
